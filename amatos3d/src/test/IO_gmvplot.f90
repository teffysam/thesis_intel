...
!---------- local declarations
	  ...
	  INTEGER (KIND = GRID_SI)                              :: i_io1
	  INTEGER (KIND = GRID_SI)                              :: i_cnt, j_cnt, &
	    i_alct, i_nnum, i_tnum, i_dim, i_tcnt, i_fst
	  REAL (KIND = GRID_SR), DIMENSION(:), ALLOCATABLE      :: r_ttmp
	  REAL (KIND = GRID_SR), DIMENSION(:,:), ALLOCATABLE    :: r_cox
	  INTEGER (KIND = GRID_SI), DIMENSION(:,:), ALLOCATABLE :: i_tets
	  INTEGER (KIND = GRID_SI), DIMENSION(:), ALLOCATABLE   :: i_ids
...
!---------- extract nodal grid data
	  ...
	  IF(i_alct /= 0) THEN
	    CALL grid_error(c_error='[plot_gmv] Could not allocate r_cox')
	  END IF
...
!---------- extract cells grid data
	  ...
	  IF(i_alct /= 0) THEN
	    CALL grid_error(c_error='[plot_gmv] Could not allocate i_tets')
	  END IF
...
!---------- the element levels
	  ...
	  IF(i_alct /= 0) THEN
	    CALL grid_error(c_error='[plot_gmv] Could not allocate i_ids')
	  END IF
...
!---------- the tracer concentration
	  ...
	  IF(i_alct /= 0) THEN
	    CALL grid_error(c_error='[plot_gmv] Could not allocate r_ttmp')
	  END IF
	  CALL grid_getinfo(p_handle, i_nnum, i_valpoint=GRID_tracer, &
	    r_nodevalues= r_ttmp)
	  WRITE(i_io1,1042)
	  WRITE(i_io1,1011) (r_ttmp(i_cnt), i_cnt=1,i_nnum)
	  CALL grid_getinfo(p_handle, i_nnum, i_valpoint=DEF_phi, &
	    r_nodevalues= r_ttmp)
	  WRITE(i_io1,1044)
	  WRITE(i_io1,1011) (r_ttmp(i_cnt), i_cnt=1,i_nnum)
	  CALL grid_getinfo(p_handle, i_nnum, i_valpoint=DEF_zeta, &
	    r_nodevalues= r_ttmp)
	  WRITE(i_io1,1045)
	  WRITE(i_io1,1011) (r_ttmp(i_cnt), i_cnt=1,i_nnum)
	  WRITE(i_io1,1049)
