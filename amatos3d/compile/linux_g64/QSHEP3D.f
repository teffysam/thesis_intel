C ADDED SOME NUMERICAL STABILITY ON SOME COMPILERS
C BY ALTERING "IF(REAL .NO .NE. 0.)" TO "IF(ABS(REAL .NO) .LT. EPS)".
C THEREFORE, WE NEED A CONSTANT EPS.
C J. BEHRENS, 15.07.2004
      FUNCTION EPS()
C THIS IS A DIRTY HACK, SINCE WE USE THE DOUBLE PRECISION EPSILON HERE<
...
C
    1	SUM = SUM + RS
	  ...
	  LNP = LNP + 1
	  RSOLD = RS
	  CALL GETNP3 (NN,XK,YK,ZK,X,Y,Z,NNR,LCELL,LNEXT,XYZMN,
     .		       XYZDL, NP,RS)
	  IF (RS .EQ. 0.) GO TO 21
	  NPTS(LNP) = NP
	  IF ( (RS-RSOLD)/RS .LT.  RTOL ) GO TO 1
	  IF (RWS .EQ. 0.  .AND.  LNP .GT. NNW) RWS = RS
	  IF (ABS(RQ) .LT. EPS  .OR.	LNP .LE. NNQ) GO TO 2
C
...
C
C INTRINSIC FUNCTIONS CALLED BY QS3VAL -- IFIX, SQRT
C
C***********************************************************
C
      XP = PX
...
C
C INTRINSIC FUNCTIONS CALLED BY QS3GRD -- IFIX, SQRT
C
C***********************************************************
C
      XP = PX
...
C
    6 Q = 0.
      ...
      RETURN
      END
      SUBROUTINE GETNP3 (N,PX,PY,PZ,X,Y,Z,NR,LCELL,LNEXT,
     .			 XYZMIN,XYZDEL, NP,DSQ)
      INTEGER  N,NR, LCELL(NR,NR,NR), LNEXT(N), NP
      REAL     PX, PY, PZ, X(N), Y(N), Z(N), XYZMIN(3),
     .	      XYZDEL(3), DSQ
C
C***********************************************************
C
...
C
C INTRINSIC FUNCTIONS CALLED BY GETNP3 -- IABS, IFIX, SQRT
C
C***********************************************************
C
      LOGICAL FIRST
...
C
C INTRINSIC FUNCTIONS CALLED BY STORE3 -- FLOAT, IFIX
C
C***********************************************************
C
      NN = N
