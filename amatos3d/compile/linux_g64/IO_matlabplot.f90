...
!---------- local declarations
	  ...
	  INTEGER (KIND = GRID_SI)                              :: i_io1, i_cnt, i_opt, i_tim, i_alct, i_enum
	  ...
	  INTEGER (KIND = GRID_SI), DIMENSION(:), ALLOCATABLE   :: i_val
...
!---------- allocate arrays and extract data
	  ...
	  IF(i_alct /= 0) THEN
	    CALL grid_error(c_error='[plot_matlab] Could not allocate r_xytmp')
	  END IF
	  ...
	  IF(l_flag) THEN
	    ...
	    IF(i_alct /= 0) THEN
	      CALL grid_error(c_error='[plot_matlab] Could not allocate (flag) i_val')
	    END IF
	    CALL grid_getinfo(p_handle, i_enum, l_finelevel=.TRUE., &
	  ...
	  ELSE IF(l_levl) THEN
	    allocate(i_val(i_enum), stat=i_alct)
	    IF(i_alct /= 0) THEN
	      CALL grid_error(c_error='[plot_matlab] Could not allocate (level) i_val')
	    END IF
	    CALL grid_getinfo(p_handle, i_enum, l_finelevel=.TRUE., &
	  ...
	  ELSE IF(l_vals) THEN
	    ...
	    IF(i_alct /= 0) THEN
	      CALL grid_error(c_error='[plot_matlab] Could not allocate r_val')
	    END IF
	    CALL grid_getinfo(p_handle, i_enum, l_finelevel=.TRUE., &
