...
!---------- allocate current data arrays
	...
	! write(*,*) i_alct, my_errmsg
     IF(i_alct /= 0) &
	  ...
	  !READ TIME
	  c_filename='/scratch/cen/numgeo/teffy/model_data_agulhas_region/agulhas_uvel.cdf'
...
countB(4)=i_timesteps
	  ...
	  IF(i_ncstat /= NF_NOERR) &
		...
		!   OPEN VVEL
		c_filename='/scratch/cen/numgeo/teffy/model_data_agulhas_region/agulhas_vvel.cdf'
      i_ncstat= nf_open(c_filename,NF_NOWRITE,i_fileid)
	  IF(i_ncstat /= NF_NOERR) &
		CALL grid_error(c_error='[read_netcdf_currents]: could not open currents data file')
	...
	!   OPEN WVEL
		c_filename='/scratch/cen/numgeo/teffy/model_data_agulhas_region/agulhas_wvel.cdf'
      i_ncstat= nf_open(c_filename,NF_NOWRITE,i_fileid)
	  IF(i_ncstat /= NF_NOERR) &
	    CALL grid_error(c_error='[read_netcdf_currents]: could not open currents data file')
...
!-----------set timeinterval length between winddata (Timeformat here: yymmdd,ddd)
	  ...
	  r_intervallen= 86400 !r_time(2) - r_time(1) ! already in seconds, timestep is 86400 anyway
...
!---------- local variables
	  ...
	  c_filename= '/scratch/cen/numgeo/teffy/model_data_agulhas_region/agulhas_grid_rdr.cdf'
!---------- open current file
...
!---------- local declarations
	  ...
	  REAL                            :: r_deg
	  REAL, PARAMETER                 :: r_earth=40075000. ! earth circumference in metres
...
!---------- calculate weights for bilinear interpolation
	  ...
	  r_dz=  abs(r_z(i_hiz) -  r_z(i_loz)) 
	  r_dwz=  abs(r_zw(i_hizz) - r_zw(i_lozz)) 
	!   write (*,*) r_z ,r_zw
	  ...
	  r_lz= abs(r_z(i_loz) - r_coord(3)) 
	  r_lzz=  abs(r_zw(i_lozz) - r_coord(3)) 
	  ...
	  r_hz=   abs(r_coord(3) -r_z(i_hiz)) 
	  r_hzz=   abs(r_coord(3) - r_zw(i_hizz)) 
	!   write (*,*) r_hz, r_hzz

	  r_scalx= r_deg * 1./cos(r_coord(2)*PI/180.)  ! in degree/sec 
	  r_scaly= r_deg
	  r_scalz= 1.  / 1000.                         ! km to m
	!   write(*,*) r_scalx, r_scaly, r_scalz
