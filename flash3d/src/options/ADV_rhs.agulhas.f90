...
!*****************************************************************
	MODULE ADV_rhs
	  ...
	  CONTAINS
!*****************************************************************
	  FUNCTION slm_ring(r_field,r_coord,i_cnt, r_time) RESULT (r_rhs)
	  IMPLICIT NONE
	  INTEGER, INTENT(in) :: i_cnt
	  REAL, DIMENSION(3,1), INTENT(in) :: r_coord
	  REAL		                       		  :: r_rhs
...
!---------- Simple Criteria for injecting tracer
	...
	!   IF (r_coord(1,i_cnt) .gt. 27.0 ) THEN
	  ...
	  FUNCTION slm_righthand(p_param,r_coord,i_cnt, r_time) RESULT (r_rhs)
...
! Just testing out changes to github
...
!	  write(*,*) r_field
	!   write(*,*) r_coord(1,i_cnt)
	!    IF (-19.0 .le. r_coord(1,i_cnt) .ge. -20.0 ) THEN
...
! Volcanic Eruptions switched off for Agulhas Eddies test
!*****************************************************************
